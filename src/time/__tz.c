...
static const unsigned char *zi, *trans, *index_tz, *types, *abbrevs, *abbrevs_end;
...
static void do_tzset()
{
	...
	if (map) {
		...
		index_tz = trans + (zi_read32(trans-12) << scale);
		types = index_tz + zi_read32(trans-12);
...
static size_t scan_trans(long long t, int local, size_t *alt)
{
	...
	size_t a = 0, n = (index_tz-trans)>>scale, m;
	...
	while (n > 1) {
		...
		if (local) off = (int32_t)zi_read32(types + 6 * index_tz[m-1]);
		if (t - off < (int64_t)x) {
			n /= 2;
		} else {
			a = m;
	...
	n = (index_tz-trans)>>scale;
	if (a == n-1) return -1;
	if (a == 0) {
		x = zi_read32(trans + (a<<scale));
		if (scale == 3) x = x<<32 | zi_read32(trans + (a<<scale) + 4);
		else x = (int32_t)x;
		if (local) off = (int32_t)zi_read32(types + 6 * index_tz[a-1]);
		if (t - off < (int64_t)x) {
	...
	if (alt) {
		if (a && types[6*index_tz[a-1]+4] != types[6*index_tz[a]+4])
			*alt = index_tz[a-1];
		else if (a+1<n && types[6*index_tz[a+1]+4] != types[6*index_tz[a]+4])
			*alt = index_tz[a+1];
		else
			*alt = index_tz[a];
	}
